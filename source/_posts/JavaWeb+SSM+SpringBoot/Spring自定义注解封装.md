...
- **第三步，解释注解——在编译期或与运行时检测到标记，并进行特殊操作。**

![image-20230228100026091](Spring自定义注解封装/image-20230228100026091-16775511049171.png)
...
也许你会认为这不就是接口中定义抽象方法的语法嘛？那继续看看下边这个：
...
可以看出，注解类型元素的语法非常奇怪即又有属性的特征（可以赋值）,又有方法的特征（打上了一对括号）。但是这么设计是有道理的，我们在后面的章节中可以看到：注解在定义好了以后，**使用的时候操作元素类型像在操作属性，解析的时候操作元素类型像在操作方法**。
